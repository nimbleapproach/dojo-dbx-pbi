...
# MAGIC %md
# MAGIC ##Process: data_quality_check_1
...
# MAGIC
# MAGIC The purpose of this test is to detect a loss of data for the current day, or a loss of historic data in mssn_wallets.
# MAGIC
...
duplication_scope = 15
duplication_tolerance = 0.005
...
historic_aggregation_query = """
                                ...
                                WHERE CAST(upd_ts AS date) <= '{{date}}' AND CAST(event_ts AS date) <= '{{date}}'
                                GROUP BY CAST(upd_ts AS date)""".format(core_catalog = core_catalog)  # {0} should be the date to conduct the test from.
