...
# MAGIC %md
# MAGIC ##Process: data_quality_check_11

# COMMAND ----------

# MAGIC %md
# MAGIC ## Tests for cmpgn_setup
# MAGIC
# MAGIC The purpose of this test is to check for a loss of historic data in cmpgn_setup.
# MAGIC
...
# MAGIC It selects historic records to ensure there is no change.
# MAGIC
...
historic_aggregation_query = """
                    WITH cte AS (SELECT *, 
                    ROW_NUMBER() OVER(PARTITION BY cmpgn_id ORDER BY load_ts ASC) AS row_number
                    FROM {core_catalog}.gb_mb_dl_tables.cmpgn_setup
                    WHERE rec_status_ind = 'HISTORIC')
                    

                    SELECT CAST(load_ts AS DATE), COUNT(*) as row_count FROM cte 
                    WHERE row_number = 1 AND CAST(load_ts AS date) <= '{{date}}'
                    GROUP BY CAST(load_ts AS DATE)
                    ORDER BY CAST(load_ts AS DATE) DESC
                    """.format(core_catalog = core_catalog)
