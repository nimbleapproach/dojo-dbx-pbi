...
# MAGIC %md
# MAGIC ##Process: data_quality_check_13

# COMMAND ----------

# MAGIC %md
# MAGIC ## Tests for coupn_wallets
# MAGIC
# MAGIC The purpose of this test is to check for a loss of historic data in coupn_wallets.
# MAGIC
...
historical_aggregation_query = """
                    SELECT CAST(load_ts AS date), 
                           COUNT(*) AS row_count FROM 
                                    {core_catalog}.gb_mb_dl_tables.coupn_wallets
                    WHERE CAST(load_ts AS date) <= '{{date}}' AND
                    rec_status_ind = 'CURRENT' AND
                    (coupn_status_desc = 'USED' or coupn_status_desc = 'CANCELLED')
                    GROUP BY CAST(load_ts AS date)
                    ORDER BY CAST(load_ts AS date) desc""".format(core_catalog = core_catalog)
