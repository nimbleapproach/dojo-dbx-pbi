...
# MAGIC %md
# MAGIC ##Process: data_quality_check_15

# COMMAND ----------

# MAGIC %md
# MAGIC ## Tests for cdd_rpt_loyalty_acct
# MAGIC
# MAGIC The purpose of this test is to detect a loss of data for the current day in gb_customer_data_domain_secured_rpt.cdd_rpt_loyalty_acct
# MAGIC
# MAGIC It has been put in place due to irregular data loads. 
# MAGIC
# MAGIC It uses the the median value to test for a low count for the day. If 'todays' count is less than 20% of the median of the distribution over the previous 25 days, then today's row count value is classed as an outlier.
# MAGIC The count is split by the column dt_cnvrt_to_full_acct_ts.
# MAGIC
# MAGIC Data gets loaded tuesday -> saturday, so we only check against the last day that data was loaded.
# MAGIC
# MAGIC This metric was selected through historical examination of the data.
# MAGIC
# MAGIC This test historically has a false positive rate of ~0.5%. Over a year this test is expected to produce ~2 false positives.
# MAGIC
...
def get_date_for_check(today_date: datetime.datetime):
    """
    At the moment data gets loaded Tuesday to saturday, so we need to check the last valid day
    This function will return the last date we expect to have full data
    Args:
        todays_date(datetime): todays date to determine the data for check
    Returns:
        date_for_check(datetime): The last date we expect to have full data 

    """
    day_of_week = today_date.weekday()
    date_for_check_modifier = {
        0: 3,
        1: 4,
        2: 2,
        3: 2,
    ...
    date_for_check = today_date - datetime.timedelta(days=date_for_check_modifier[day_of_week])
...
daily_query = f"""
                SELECT CAST(dt_cnvrt_to_full_acct_ts AS date) AS dt,
                COUNT(*) AS row_count 
                FROM {core_catalog}.gb_customer_data_domain_secured_rpt.cdd_rpt_loyalty_acct
                WHERE CAST(dt_cnvrt_to_full_acct_ts AS date) > 
                        DATE_ADD(CURRENT_DATE(), -{prev_day})
                GROUP BY CAST(dt_cnvrt_to_full_acct_ts AS date)
                ORDER BY CAST(dt_cnvrt_to_full_acct_ts AS date) DESC"""


# COMMAND ----------

#Get date for check
date_for_check = get_date_for_check(datetime.datetime.today()).strftime('%Y-%m-%d')
