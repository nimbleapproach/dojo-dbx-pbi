...
# MAGIC %md
# MAGIC ##Process: data_quality_check_16

# COMMAND ----------

# MAGIC %md
# MAGIC ## Test for cdd_odl_singl_profl_customer
# MAGIC
# MAGIC The purpose of this test is to detect a loss of data for the current day in gb_customer_data_domain_secured_odl.cdd_odl_singl_profl_customer
# MAGIC
# MAGIC It has been put in place due to irregular data loads. 
# MAGIC
# MAGIC It uses the the median value to test for a low count for the day. If 'todays' count is less than 30% of the median of the distribution over the previous 25 days, then today's row count value is classed as an outlier.
# MAGIC The count is split by the column ingest_ts. Since cdd tables load late, we check the row count for 2 days previously. For example, runnning the check on 18/01/2024, will check the row counts for 16/01/2024.
# MAGIC
...
daily_query = f"""
            SELECT CAST(ingest_ts AS date) AS dt,
            COUNT(*) AS row_count 
            FROM {core_catalog}.gb_customer_data_domain_secured_odl.cdd_odl_singl_profl_customer
            WHERE CAST(ingest_ts AS date) > 
                    DATE_ADD(CURRENT_DATE(), -{prev_day})
            GROUP BY CAST(ingest_ts AS date)
            ORDER BY CAST(ingest_ts AS date) DESC"""
