...
# MAGIC %md
# MAGIC ##Process: data_quality_check_19

# COMMAND ----------

# MAGIC %md
# MAGIC ## Tests for groc_order_rfnd
# MAGIC
# MAGIC The purpose of this test is to detect a loss of data for the current day, or a loss of historic data in groc_order_rfnd.
# MAGIC
# MAGIC It has been put in place due to irregular data loads in groc_order_rfnd, often lacking data for the current day. 
# MAGIC
# MAGIC The daily test compares the row counts for the current day to the median value over the previous 25 days. The row count is deemed to be 'low' if it is less than 30% of the median value over this window. 
# MAGIC Due to late date loads the row counts are checked for 2 days prior, as this is the data loaded on the current day.
# MAGIC
...
daily_query = f"""
                SELECT CAST(src_create_ts AS date) AS dt,
                count(*) as row_count
                FROM {core_catalog}.gb_mb_dl_tables.groc_order_rfnd
                WHERE CAST(src_create_ts AS date) > 
                        DATE_ADD(CURRENT_DATE(), -{prev_day})
                GROUP BY CAST(src_create_ts AS date)
                ORDER BY CAST(src_create_ts AS date) DESC"""

historical_aggregation_query = """
                    SELECT CAST(src_create_ts AS date), 
                           COUNT(*) AS row_count FROM 
                                    {core_catalog}.gb_mb_dl_tables.groc_order_rfnd
                    WHERE src_create_ts <= '{{date}}'
                    GROUP BY CAST(src_create_ts AS date)""".format(core_catalog = core_catalog)
