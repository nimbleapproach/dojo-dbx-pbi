...
# MAGIC %md
# MAGIC ##Process: data_quality_check_23

# COMMAND ----------

# MAGIC %md
# MAGIC ## Tests for ghs_order_kafka
# MAGIC
# MAGIC The purpose of this test is to detect a loss of data for the current day, a loss of historic data or duplicate data in ghs_order_kafka.
# MAGIC
# MAGIC It has been put in place due to irregular data loads in wallet_pos_txns, often lacking data for the current day. 
# MAGIC
# MAGIC The daily test compares the row counts for the current day to the median value over the previous 20 days. The row count is deemed to be 'low' if it is less than 60% of the median value over this window. 
# MAGIC
...
daily_query = f"""
            SELECT CAST(dlvr_dt AS date) AS dt,
                    COUNT(*) AS row_count 
            FROM {core_catalog}.gb_mb_secured_dl_tables.ghs_order_kafka 
            WHERE CAST(dlvr_dt AS date) > 
                DATE_ADD(CURRENT_DATE(), -{prev_day}) AND
                order_status_nm = 'NO_PENDING_ACTION'
            GROUP BY CAST(dlvr_dt AS date)
            ORDER BY CAST(dlvr_dt AS date) DESC """

duplication_query = f"""
                    SELECT CAST(dlvr_dt as date) AS dt, COUNT(web_order_id) AS web_id_count, count(*) AS row_count FROM {core_catalog}.gb_mb_secured_dl_tables.ghs_order_kafka
                        WHERE order_status_nm = "NO_PENDING_ACTION" AND CAST(dlvr_dt as date) > DATE_ADD(CURRENT_DATE(), -{duplication_scope}) AND
                        CAST(dlvr_dt AS date) < CURRENT_DATE()
                    GROUP BY CAST(dlvr_dt as date)
                    ORDER BY cast(dlvr_dt as date) DESC"""

historical_aggregation_query = """
                    SELECT CAST(dlvr_dt AS date), 
                           COUNT(*) AS row_count FROM 
                                    {core_catalog}.gb_mb_secured_dl_tables.ghs_order_kafka
                    WHERE CAST(dlvr_dt AS date) <= '{{date}}' AND
                    order_status_nm = "NO_PENDING_ACTION"
                    GROUP BY CAST(dlvr_dt AS date)""".format(core_catalog = core_catalog)
