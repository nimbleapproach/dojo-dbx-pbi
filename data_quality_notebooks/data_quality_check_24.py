...
# MAGIC %md
# MAGIC ##Process: data_quality_check_24

# COMMAND ----------

# MAGIC %md
# MAGIC ## Tests for mssn_wallets
# MAGIC
# MAGIC The purpose of this test is to detect a loss of data for the current day in mssn_wallets. The test will be used as a warning, so it will have strict parameters
# MAGIC
# MAGIC It has been put in place due to irregular data loads in mssn_wallets, often lacking data for the current day. 
# MAGIC
# MAGIC The daily test compares the row counts for the current day to the median value over the previous 40 days. The row count is deemed to be 'low' if it is less than 40% of the median value over this window. 
# MAGIC
