...
# MAGIC %md
# MAGIC ##Process: data_quality_check_32

# COMMAND ----------

# MAGIC %md
# MAGIC ## Tests for cdd_odl_pfs_sales
# MAGIC
# MAGIC The purpose of this test is to detect a loss of data for the current day.
# MAGIC
# MAGIC It has been put in place due to irregular data loads in cdd_odl_pfs_sales, often lacking data for the current day. 
# MAGIC
# MAGIC The daily test compares the counts for the current day to the median value over the previous 20 days. The row count is deemed to be 'low' if it is less than 70% of the median value over this window. 
# MAGIC
...
daily_query_fuel1 = f"""
                SELECT visit_dt AS dt,
                        COUNT(DISTINCT wallet_id) AS reward_custs 
                FROM {core_catalog}.gb_customer_data_domain_odl.cdd_odl_pfs_sales
                WHERE visit_dt > 
...
daily_count_check_result_fuel1, error_message = run_daily_count_test(query = daily_query_fuel1,
                                                               metric = daily_metric, 
                                                               date_for_check = date_for_check, 
                                                               tolerance = daily_tolerance,
                                                               spark = spark)

#If previous test passes test for second daily test:
if daily_count_check_result_fuel1 == True:
    daily_count_check_result_fuel2, error_message = run_daily_count_test(query = daily_query_fuel2,
                                                               metric = daily_metric, 
                                                               date_for_check = date_for_check, 
                                                               tolerance = daily_tolerance,
                                                               spark = spark)
    status = (daily_count_check_result_fuel1 & daily_count_check_result_fuel2)
