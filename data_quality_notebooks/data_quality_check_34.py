...
# MAGIC %md
# MAGIC ##Process: data_quality_check_34

# COMMAND ----------

# MAGIC %md
# MAGIC ## Tests for cdd_rpt_lylty_supp_bill
# MAGIC
# MAGIC The purpose of this test is to detect a loss of data for the current days data load.
# MAGIC
# MAGIC The daily test compares the counts for the current day to the median value over the previous 20 days. The row count is deemed to be 'low' if it is less than 30% of the median value over this window. 
# MAGIC
# MAGIC
# MAGIC Since this is a cdd table, the 'current load' of data will contain data for two days prior. For example, a check on 29/02/2024 will check for daat on 27/02/2024
# MAGIC
...
daily_query = f"""
                SELECT visit_dt AS dt,
                        count(*) as row_count
                FROM {core_catalog}.gb_customer_data_domain_rpt.cdd_rpt_lylty_supp_bill
                WHERE visit_dt > 
...
if before_midday:
    # date for check
    date_for_check = ((datetime.datetime.today() - datetime.timedelta(days=3))
                            .strftime('%Y-%m-%d'))
else:
