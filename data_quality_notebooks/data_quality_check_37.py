...
# MAGIC %md
# MAGIC ##Process: data_quality_check_37

# COMMAND ----------

# MAGIC %md
# MAGIC ## Tests for cdd_odl_wallet_pos_txns
# MAGIC
# MAGIC The purpose of this test is to detect a loss of data for the current day, a loss of historic data or duplicate data in cdd_odl_wallet_pos_txns.
# MAGIC
# MAGIC It has been put in place due to irregular data loads in wallet_pos_txns, often lacking data for the current day. 
# MAGIC
# MAGIC The daily test compares the row counts for the current day to the median value over the previous 15 days. The row count is deemed to be 'low' if it is less than 55% of the median value over this window. 
# MAGIC
...
duplication_scope = 365 #Number of days historically we check for duplicates
...
daily_query = f"""
                SELECT CAST(event_ts AS date) AS dt,
                        COUNT(*) AS row_count 
                FROM {core_catalog}.gb_customer_data_domain_odl.cdd_odl_wallet_pos_txns
...
historical_aggregation_query = """
                    SELECT CAST(event_ts AS date), 
                           COUNT(*) AS row_count FROM 
                                    {core_catalog}.gb_customer_data_domain_odl.cdd_odl_wallet_pos_txns
