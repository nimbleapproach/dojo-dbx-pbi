...
# MAGIC %md
# MAGIC ## Tests for cdd_rpt_transaction_item
# MAGIC
# MAGIC The purpose of this test is to detect a loss of data for the current day in cdd_rpt_transaction_item
# MAGIC
...
daily_count_query = f"""
                    SELECT CAST(visit_ts AS date) AS dt,
                            COUNT(*) AS row_count 
                    FROM {core_catalog}.gb_customer_data_domain_rpt.cdd_rpt_transaction_item
                    WHERE CAST(visit_ts AS date) > 
                        DATE_ADD(CURRENT_DATE(), -30)
                    GROUP BY CAST(visit_ts AS date)
                    ORDER BY CAST(visit_ts AS date) DESC"""
