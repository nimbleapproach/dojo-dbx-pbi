...
# MAGIC %md
# MAGIC ##Process: data_quality_check_45

# COMMAND ----------

# MAGIC %md
# MAGIC ## Test datamart table ar_cust_full_visit_history, chnl_nm ecom
# MAGIC
# MAGIC This notebook checks whether the the table ar_cust_full_visit_history, with chnl_nm ecom, has data for the current day -2
# MAGIC
...
day_minus_2 = ((datetime.datetime.today() - datetime.timedelta(days=2))
                          .strftime('%Y-%m-%d'))
day_minus_1 = ((datetime.datetime.today() - datetime.timedelta(days=1))
                          .strftime('%Y-%m-%d'))
...
date_column_query = f"""
                    SELECT CAST(visit_dt AS date) AS dt FROM {catalog}.cdna_ar.ar_cust_full_visit_history
                    WHERE chnl_nm = 'ecom'
                    GROUP BY CAST(visit_dt AS date)
                    ORDER BY CAST(visit_dt AS date) DESC
                    limit 20
"""
...
try:
    status = date_column_check(date_column_query, day_minus_2, spark) | date_column_check(date_column_query, day_minus_1, spark)
