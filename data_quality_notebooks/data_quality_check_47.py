...
# MAGIC %md
# MAGIC ##Process: data_quality_check_47

# COMMAND ----------

# MAGIC %md
# MAGIC ## Test datamart table cdna_ar.dsa_ar_customer_smry 
# MAGIC
# MAGIC This notebook checks whether the the table cdna_ar.dsa_ar_customer_smry has data for the past week
# MAGIC
...
date_column_query = f"""
                    SELECT CAST(first_visit_dt AS date) AS dt FROM {catalog}.cdna_ar.dsa_ar_customer_smry
                    GROUP BY CAST(first_visit_dt AS date)
                    ORDER BY CAST(first_visit_dt AS date) DESC
                    limit 20
"""
...
try:
    ...
    #Get the current and last week number:
    # Current week number:
    current_week = datetime.date.today().strftime("%V")
    # number of prev week
    previous_week = (datetime.date.today()+datetime.timedelta(weeks=-1)).strftime("%V")

    # Compare week numbers:
    if previous_week == most_recent_date.strftime("%V"):
        status = True
    elif current_week == most_recent_date.strftime("%V"):
        status = True
    else:
        status = False
