...
# MAGIC %md
# MAGIC ##Process: data_quality_check_48

# COMMAND ----------

# MAGIC %md
# MAGIC ## Test datamart table ar_cust_rewards_burn
# MAGIC
# MAGIC This notebook checks whether the the table ar_cust_rewards_burn has data from the previous friday
# MAGIC
...
date_for_check = (current_time.date()
    - datetime.timedelta(days=current_time.weekday())
    + datetime.timedelta(days=4, weeks=-1)).strftime('%Y-%m-%d')
...
date_column_query = f"""
                    SELECT CAST(redm_visit_date AS date) AS dt FROM {catalog}.cdna_ar.ar_cust_rewards_burn
                    GROUP BY CAST(redm_visit_date AS date)
                    ORDER BY CAST(redm_visit_date AS date) DESC
                    limit 20
"""
...
try:
   status = date_column_check(date_column_query, date_for_check, spark)
