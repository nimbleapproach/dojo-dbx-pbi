...
# MAGIC %md
# MAGIC ##Process: data_quality_check_49

# COMMAND ----------

# MAGIC %md
# MAGIC ## Test datamart table dsa_helper_last_wk
# MAGIC
# MAGIC This notebook checks whether the the table dsa_helper_last_wk exists
# MAGIC
...
# MAGIC
# MAGIC Query should produce a column run_week with 1 row

# COMMAND ----------

table_check_query = f"""
                    SELECT run_week FROM {catalog}.cdna_ar.dsa_helper_last_wk
"""
...
# MAGIC %md
# MAGIC
# MAGIC #### Process for Testing table exists:
...
try:
    cursor = spark.sql(table_check_query)
    rows = cursor.collect()

    if not rows:
        status = False
        table_exists = False
        error_message = "Table exists but is empty."
    else:
        status = True
        table_exists = True
...
if table_exists and status:
    print("Test Passed: Table exists and has data.")
elif table_exists and not status:
    print("Test Failed: Table exists but is empty.")
elif error_message:
