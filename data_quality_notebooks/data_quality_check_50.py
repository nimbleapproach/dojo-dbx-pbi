...
# MAGIC %md
# MAGIC ##Process: data_quality_check_50

# COMMAND ----------

# MAGIC %md
# MAGIC ## Test datamart table cdna_ar.dsa_helper_week_finder
# MAGIC
# MAGIC This notebook checks whether the the table cdna_ar.dsa_helper_week_finder has data for the past week
# MAGIC
...
date_column_query = f"""
                    SELECT CAST(week_end_day AS date) as dt FROM {catalog}.cdna_ar.dsa_helper_week_finder
                    GROUP BY CAST(week_end_day AS date)
                    ORDER BY CAST(week_end_day AS date) desc
                    limit 10
"""
