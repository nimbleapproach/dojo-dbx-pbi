...
# MAGIC %md
# MAGIC ##Process: data_quality_check_52

# COMMAND ----------

# MAGIC %md
# MAGIC ## Test datamart table cdna_ar.v_stg_01_cust_attrib_extended
# MAGIC
# MAGIC This notebook checks whether the the table cdna_ar.v_stg_01_cust_attrib_extended has any data
...
query = f"""
                    SELECT * FROM {catalog}.cdna_ar.v_stg_01_cust_attrib_extended
                    LIMIT 5
"""
...
try:
   query_result = spark.sql(query).cache()
   display(query_result)
   if len(query_result.collect()) == 5:
       status = True
   else:
       status = False
