...
# MAGIC %md
# MAGIC ##Process: data_quality_check_6

# COMMAND ----------

# MAGIC %md
# MAGIC ## Tests for ext_trans.
# MAGIC
# MAGIC The purpose of this test is to detect a loss of data for the current day, or a loss of historic data in ext_trans.
# MAGIC
# MAGIC It has been put in place due to irregular data loads in ext_trans, often lacking data for the current day. 
# MAGIC
# MAGIC The daily test compares the row counts for the current day to the median value over the previous 15 days. The row count is deemed to be 'low' if it is less than 15% of the median value over this window. 
# MAGIC
...
daily_query = f"""
                SELECT CAST(event_ts AS date) AS dt,
                        COUNT(*) AS row_count 
                FROM {core_catalog}.gb_mb_dl_tables.ext_trans 
...
historical_aggregation_query = """
                    SELECT CAST(upd_ts AS date), 
                           COUNT(*) AS row_count FROM 
                                    {core_catalog}.gb_mb_dl_tables.ext_trans
                    WHERE CAST(upd_ts AS date) <= '{{date}}'
