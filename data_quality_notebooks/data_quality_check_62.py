...
# MAGIC %md
# MAGIC ##Process: data_quality_check_62

# COMMAND ----------

# MAGIC %md
# MAGIC ## Test datamart table ar_cust_earning
# MAGIC
# MAGIC This notebook checks whether the the table ar_cust_earning has data for the current day -2
# MAGIC
...
date_column_query = f"""
                    SELECT CAST(cmpgn_cmplt_ts AS date) AS dt FROM {catalog}.cdna_ar.ar_cust_earning
                    GROUP BY CAST(cmpgn_cmplt_ts AS date)
                    ORDER BY CAST(cmpgn_cmplt_ts AS date) DESC
                    limit 20
"""
