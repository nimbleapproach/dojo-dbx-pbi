...
# MAGIC %md
# MAGIC ##Process: data_quality_check_67

# COMMAND ----------

# MAGIC %md
# MAGIC ## Tests for gb_wm_mb_vm.visit
# MAGIC
# MAGIC The purpose of this test is to detect a loss of data for the current day, for gb_wm_mb_vm.visit
# MAGIC
...
daily_query = f"""
                SELECT CAST(visit_date AS date) AS dt,
                        COUNT(*) AS row_count 
                FROM {core_catalog}.gb_wm_mb_vm.visit
                WHERE CAST(visit_date AS date) > 
                    DATE_ADD(CURRENT_DATE(), -{prev_day})
                AND CAST(visit_date AS date) <= CURRENT_DATE()
