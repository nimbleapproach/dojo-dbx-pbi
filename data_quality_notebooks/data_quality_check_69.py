...
# MAGIC %md
# MAGIC ##Process: data_quality_check_69

# COMMAND ----------

# MAGIC %md
# MAGIC ## Tests for gb_customer_data_domain_odl.cdd_odl_loyalty_acct
# MAGIC
# MAGIC The purpose of this test is to detect a loss of data for the current day in gb_customer_data_domain_odl.cdd_odl_loyalty_acct.
# MAGIC
# MAGIC It has been put in place due to irregular data loads in gb_customer_data_domain_odl.cdd_odl_loyalty_acct, often lacking data for the current day. 
# MAGIC
...
daily_count_query = f"""
                    ...
                    count(*) as row_count
                    FROM
                    {core_catalog}.gb_customer_data_domain_odl.cdd_odl_loyalty_acct
                    WHERE
                    cast(event_ts as date) >= dateadd(GETDATE(),-{prev_day})
