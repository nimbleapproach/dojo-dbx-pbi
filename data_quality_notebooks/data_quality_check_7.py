...
# MAGIC %md
# MAGIC ##Process: data_quality_check_7

# COMMAND ----------

# MAGIC %md
# MAGIC ## Tests for wallet_pos_txns
# MAGIC
# MAGIC The purpose of this test is to detect a loss of data for the current day, a loss of historic data or duplicate data in wallet_pos_txns, specifically for PFS transactions.
# MAGIC
# MAGIC It has been put in place due to irregular data loads in wallet_pos_txns, often lacking data for the current day. 
# MAGIC
# MAGIC The daily test compares the row counts for the current day to the median value over the previous 20 days. The row count is deemed to be 'low' if it is less than 40% of the median value over this window. 
# MAGIC
...
daily_tolerance = 0.70
daily_metric = PercentageOfMedian()

duplication_tolerance = 0.001
duplication_scope = 365 #Number of days we check historically for duplicates.
...
daily_query = f"""
                ...
                WHERE visit_dt > DATE_ADD(CURRENT_DATE(), -{prev_day}) AND
                store_nbr = 4582 AND
                chnl_nm LIKE '%pfs%'
                GROUP BY visit_dt
...
duplication_query = f"""
                    SELECT CAST(event_ts AS date) AS dt, count(distinct(trans_rcpt_nbr)) as trans_rcpt_nbrs,
                    COUNT(*) AS row_count 
                    FROM {core_catalog}.gb_mb_dl_tables.wallet_pos_txns
                    WHERE chnl_nm = "ecom" AND CAST(event_ts as date) > DATE_ADD(CURRENT_DATE(), -{duplication_scope}) AND
...
historical_aggregation_query = """
                    SELECT CAST(event_ts AS date), 
                           COUNT(*) AS row_count FROM 
                                    {core_catalog}.gb_mb_dl_tables.wallet_pos_txns
