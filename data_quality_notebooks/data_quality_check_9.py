...
# MAGIC %md
# MAGIC ##Process: data_quality_check_9

# COMMAND ----------

# MAGIC %md
# MAGIC ## Tests for reward_wallets
# MAGIC
# MAGIC The purpose of this test is to detect a loss of data for the current day, or a loss of historic data in reward_wallets.
# MAGIC
# MAGIC It has been put in place due to irregular data loads in reward_wallets, often lacking data for the current day. 
# MAGIC
...
daily_query = f"""
                SELECT CAST(event_ts AS date) AS dt,
                        COUNT(*) AS row_count 
                FROM {core_catalog}.gb_mb_dl_tables.reward_wallets 
...
duplication_query = f"""
                    WITH cte_distinct AS (
                        SELECT distinct acct_id,wallet_id,cashpot_id,cmpgn_id,cmpgn_type_desc,reward_status_desc,reward_gained_ts,reward_end_ts,reward_redm_ts,rec_status_ind,event_ts,trans_rcpt_nbr FROM {core_catalog}.gb_mb_dl_tables.reward_wallets
                        WHERE CAST(event_ts AS date) > date_add(CURRENT_DATE(), -{duplication_scope}) AND
                        CAST(event_ts AS date) < CURRENT_DATE()
                    ), cte AS (
                        SELECT * FROM {core_catalog}.gb_mb_dl_tables.reward_wallets
                        WHERE CAST(event_ts as date) > date_add(CURRENT_DATE(), -{duplication_scope}) AND
                        CAST(event_ts AS date) < CURRENT_DATE()
                    ), distinct_counts AS (
...
historical_aggregation_query = """
                    SELECT CAST(event_ts AS date), 
                           COUNT(*) AS row_count FROM 
                                    {core_catalog}.gb_mb_dl_tables.reward_wallets
