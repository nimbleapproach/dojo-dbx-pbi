...
# MAGIC CREATE WIDGET TEXT view_name DEFAULT "vw_active_users_funnel";
...
# MAGIC         , datediff(current_date(), min(cast(wpt.event_ts as date)) over(PARTITION BY wpt.walleT_id)) as days_since_first_scan
...
# MAGIC     WHERE wpt.event_ts < current_date()--between date_add(current_date(),-7*26) and date_add(current_date(),-1)
# MAGIC )
# MAGIC , week_totals as (
# MAGIC SELECT --event_ts
# MAGIC     cast(ceil(cast(datediff(current_date(),event_ts) as float)/7) as int) as weeks_ago
